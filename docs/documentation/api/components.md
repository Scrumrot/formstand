...
| `isClearableSchema` | `(schema: z.ZodType) => boolean` | whether the schema accepts its own `emptyValue` as input (optional, nullable, or defaulted) — the rule behind [`useField().clearable`](./hooks#usefieldreturn-tvalue) |
