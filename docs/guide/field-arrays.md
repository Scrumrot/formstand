...
With a `Form<TSchema>` and a typed path (including template paths like `` `albums.${index}.tracks` ``), `TItem` is **inferred from the schema** — no type argument. The explicit `useFieldArray<TItem>(form, path)` form is for schema-less `FieldFormApi` forms (there is nothing to infer from); passing it alongside a typed form is a compile error that tells you the fix — the path argument is blamed with `"Remove the explicit type argument: a schema-typed form infers the item type from the path"` (see [Typed paths](./typed-paths#paths-are-inferred-from-the-schema)). Dynamic paths via a selector function return `UseFieldArrayReturn<unknown>`, like `useField`.
