...
> **The `--form-prop` + `--layout module` contract: the `form` prop must BE the module's exported singleton.** The module's field hooks are pre-wired to the singleton exported from `./hooks` (e.g. `profileForm`) — the `form` prop only drives the component's shell (submit/subscription). Passing a *different* form of the same schema compiles, but silently splits state: the shell reads your instance while every field keeps reading the module's own form. The generated component therefore warns in development (`NODE_ENV !== "production"`) whenever it renders with a form that is not the module singleton. If you need a per-mount form instance instead of a singleton, use `--layout single` with `--form-prop`, or `useForm` + `createFormContext` by hand.
