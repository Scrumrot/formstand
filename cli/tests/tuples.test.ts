...
describe("tuples — single-file rendering", () => {
  ...
  // An array whose item is itself non-scalar used to degrade the row to a
  // TODO; since the row-containers work, an array item extracts an inner
  // Rows component and a tuple item binds its positions inline
  // (rowContainers.test.ts pins the full matrix).
  it("array-of-array rows extract and array-of-tuple rows bind inline", () => {
    const c = emitPlain(
      z.object({
    ...
    expect(c).toContain("useFieldArray(form, `matrix.${p0}`)");
    expect(c).toContain("path={`points.${index}.0`}");
  });
});
